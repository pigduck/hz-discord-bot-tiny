...
class Hz_Discord_Bot_Tiny_Public
{
    ...
    public function handle_order_interactions(WP_REST_Request $request)
    {
        try {
            $signature = isset($_SERVER['HTTP_X_SIGNATURE_ED25519'])
                ? sanitize_text_field($_SERVER['HTTP_X_SIGNATURE_ED25519'])
                : '';
            $timestamp = isset($_SERVER['HTTP_X_SIGNATURE_TIMESTAMP'])
                ? sanitize_text_field($_SERVER['HTTP_X_SIGNATURE_TIMESTAMP'])
                : '';
    ...
    )
    {
        ...
        $order_command_data = [
            'name' => 'order',
            'description' => __("原來你也玩原神", 'hz-discord-bot-tiny'),
            "options" => [
                [
                    "name" => "order_id",
                    "description" => __("Enter Order ID (required)", 'hz-discord-bot-tiny'),
                    "type" => 3,
                    "required" => true
                ],
                [
                    "name" => "status",
                    "description" => __("Select the order status to be changed (optional)", 'hz-discord-bot-tiny'),
                    "type" => 3,
                    "required" => false,
                    "choices" => $choices
                ],
                [
                    "name" => "note",
                    "description" => __("Enter the notes you want to add (optional)", 'hz-discord-bot-tiny'),
                    "type" => 3,
                    "required" => false
                ],
            ]
        ...
        $phone_command_data = [
            'name' => 'phone',
            'description' => __("Enter your phone number", 'hz-discord-bot-tiny'),
            "options" => [
                [
                    "name" => "phone",
                    "description" => __("Enter your phone number (required)", 'hz-discord-bot-tiny'),
                    "type" => 3,
                    "required" => true
                ],
                [
                ...
                [
                    "name" => 'payment',
                    "description" => __("Select the payment method (optional)", 'hz-discord-bot-tiny'),
                    "type" => 3,
                    "required" => false,
                    "choices" => $payment
        ...
        $products_command_data = [
            'name' => 'product',
            'description' => __("Enter your product name", 'hz-discord-bot-tiny'),
            "options" => [
                [
                    "name" => "product",
                    "description" => __("Enter your product name (required)", 'hz-discord-bot-tiny'),
                    "type" => 3,
                    "required" => true,
                    "autocomplete" => true
                ],
                [
                    "name" => "status",
                    "description" => __("Select the status to be changed (optional)", 'hz-discord-bot-tiny'),
                    "type" => 3,
                    "required" => false,
                    "choices" => [
                        ["name" => __("Enable", 'hz-discord-bot-tiny'), "value" => "enable"],
                        ["name" => __("Disable", 'hz-discord-bot-tiny'), "value" => "disable"],
                    ]
                ],
                [
                    "name" => "regular_price",
                    "description" => __("Enter the price to be changed (optional)", 'hz-discord-bot-tiny'),
                    "type" => 10,
                    "required" => false,
                ],
                [
                ...
                [
                    "name" => "stock",
                    "description" => __("Enter the stock quantity to be changed (optional)", 'hz-discord-bot-tiny'),
                    "type" => 10,
                    "required" => false,
                ]
            ]
    ...
    public function handle_general_order_command($order_id, $status, $note, $member)
    {
        ...
        if ($status) {
            $order->update_status(
                $status,
                sprintf(
                    __('The Order has been updated to %s via Discord interaction - by %s', 'hz-discord-bot-tiny'),
    ...
    public function handle_find_order_by_products_command($request_data)
    {
        ...
        if ($hz_discord_bot_form_product_enable != '1') {
            wp_send_json([
                'type' => 4,
                'data' => [
                    'content' => __("Product command not yet activated", 'hz-discord-bot-tiny')
                ]
            ]);
        }
        ...
        if (!$product) {
            wp_send_json([
                'type' => 4,
                'data' => [
                    'content' => __("Can't find the product", 'hz-discord-bot-tiny')
                ]
            ]);
        }
            ...
            if ($type == 'simple') {
                ...
                if ($status) {
                    if ($status == 'enable') {
                        ...
                        $feedback[] = __('Product has been updated for release', 'hz-discord-bot-tiny');
                    } else {
                        $product->set_status('draft');
                        $feedback[] = __('Product has been updated for draft', 'hz-discord-bot-tiny');
                    ...
                    if ($regular_price > 0) {
                        if ($sale_price) {
                            if ($regular_price > $sale_price) {
                                ...
                                $feedback[] = __('Product Regular price has been updated', 'hz-discord-bot-tiny');
                            } else {
                                $feedback[] = __('Regular price must be greater than sale price', 'hz-discord-bot-tiny');
                                $has_error++;
                            }
                        } else {
                            if ($regular_price > $product->get_regular_price()) {
                                ...
                                $feedback[] = __('Regular price must be greater than current regular price', 'hz-discord-bot-tiny');
                                $has_error++;
                            }
                        }
                    } else {
                        $feedback[] = __('Regular price must be greater than 0', 'hz-discord-bot-tiny');
                        $has_error++;
                    }
                }
                if ($sale_price) {
                    if ($sale_price > 0) {
                        if ($regular_price) {
                            if ($sale_price < $regular_price) {
                                ...
                                $feedback[] = __('Product Sale price has been updated', 'hz-discord-bot-tiny');
                            } else {
                                $feedback[] = __('Sale price must be less than the provided regular price', 'hz-discord-bot-tiny');
                                $has_error++;
                            }
                        } elseif ($sale_price < $product->get_regular_price()) {
                            $product->set_sale_price($sale_price);
                            $feedback[] = __('Product Sale price has been updated using the product\'s regular price', 'hz-discord-bot-tiny');
                        } else {
                            $feedback[] = __('Sale price must be less than the product\'s regular price', 'hz-discord-bot-tiny');
                            $has_error++;
                        }
                    } else {
                        $feedback[] = __('Sale price must be greater than 0', 'hz-discord-bot-tiny');
                        $has_error++;
                    }
                }
                //修改庫存數量
                if (isset($stock)) {
                    ...
                    $feedback[] = __('Product stock quantity has been updated', 'hz-discord-bot-tiny');
                }
                $product->save();
                ...
                if ($regular_price) {
                    $feedback[] = __('Variable products cannot be updated with regular price', 'hz-discord-bot-tiny');
                    $has_error++;
                }
                if ($sale_price) {
                    $feedback[] = __('Variable products cannot be updated with sale price', 'hz-discord-bot-tiny');
                    $has_error++;
                }
                if ($stock) {
                    $feedback[] = __('Variable products cannot be updated with stock quantity', 'hz-discord-bot-tiny');
                    $has_error++;
                }
                $product->save();
    ...
    )
    {
        ...
        $order->update_status(
            $status_key,
            __('The order has been updated to Pending through Discord interaction.', 'hz-discord-bot-tiny')
        );

        $order->add_order_note(
            sprintf(
                __('%s (%s) updated the order status to %s through Discord interaction', 'hz-discord-bot-tiny'),
