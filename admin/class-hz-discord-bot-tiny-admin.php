...
class Hz_Discord_Bot_Tiny_Admin
{
    ...
    public function enqueue_scripts($hook)
    {
        if ($hook === 'woocommerce_page_hz_discord_bot_tiny') {
            ...
            wp_localize_script($this->plugin_name, 'hz_discord_bot_tiny_language', array(
                'clear' => __('Clear', 'hz-discord-bot-tiny'),
                'clearing' => __('Clearing...', 'hz-discord-bot-tiny'),
                'clear_success' => __('All commands cleared successfully', 'hz-discord-bot-tiny'),
                'clear_failed' => __('Clear Error', 'hz-discord-bot-tiny'),
                'syncing' => __('Syncing...', 'hz-discord-bot-tiny'),
                'sync_success' => __('Command synchronization successful', 'hz-discord-bot-tiny'),
                'sync_failed' => __('Command synchronization failed', 'hz-discord-bot-tiny'),
                'sync' => __('Sync Command', 'hz-discord-bot-tiny'),
            ));
    ...
    public function admin_notices()
    {
        if ($error_message = get_transient('hz_discord_bot_settings_error')) {
            ...
            <div class="notice notice-error is-dismissible">
                <p><?php echo __('Error in sending POST request: ', 'hz-discord-bot-tiny') . esc_attr($error_message); ?></p>
            </div>
            <?php
        ...
        if (isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true') {
            add_settings_error(
                'hz_discord_bot_messages',
                'hz_discord_bot_message',
                __('The setting has been successfully saved!', 'hz-discord-bot-tiny'),
                'updated'
            );
        }
    ...
    public function send_order_to_discord_webhook($order_id)
    {
        ...
        foreach ($webHook as $config) {
            if (in_array('new', $config['type'], true) && in_array($payment_method, $config['payment'], true)) {
                $payload = json_encode(['username' => get_option('blogname'), 'content' => $this->_replacement_message($config['message'], $order)]);
    ...
    public function send_user_register_to_discord_webhook($user_id)
    {
        ...
        foreach ($webHook as $config) {
            if (in_array('register', $config['type'], true)) {
                $payload = json_encode(['username' => get_option('blogname'), 'content' => $this->_replacement_new_user_message($config['message'], $user)]);
                $webhooks = $config['webhook'];
                if (filter_var($webhooks, FILTER_VALIDATE_URL)) {
                    wp_remote_post($webhooks, [
                        'body' => $payload,
    ...
    public function send_order_status_to_discord_bot($order_id, $old_status, $new_status)
    {
        ...
        foreach ($saved_blocks as $index => $block) {
            if (in_array('wc-' . $new_status, $block['status'], true) && in_array($payment_method, $block['payment'], true)) {
                ...
                $components[] = [
                    'type' => 2,
                    'label' => __('View Order', 'hz-discord-bot-tiny'),
                    'style' => 5,
                ...
                $order_details_embed = [
                    ...
                    "fields" => [
                        [
                            "name" => __('Order Number', 'hz-discord-bot-tiny'),
                            "value" => $order->get_id(),
                            "inline" => true
                        ],
                        [
                            "name" => __('Order Status', 'hz-discord-bot-tiny'),
                            "value" => wc_get_order_status_name($new_status),
                            "inline" => true
                        ],
                        [
                            "name" => __('Payment Amount', 'hz-discord-bot-tiny'),
                            "value" => $order->get_total(),
                            "inline" => true
                        ],
                        [
                            "name" => __('Message', 'hz-discord-bot-tiny'),
    ...
    public function plugin_action_links($links)
    {
        return array_merge(array(
            '<a href="' . admin_url('admin.php?page=hz_discord_bot_tiny') . '">' . __('Settings', 'hz-discord-bot-tiny') . '</a>',
        ), $links);
    }
    ...
    private function _register_setting_commands()
    {
        ...
        add_settings_section(
            ...
            __('Order Application Commands', 'hz-discord-bot-tiny'),
            function () {
                _e('You can select the state to be controlled by the ', 'hz-discord-bot-tiny');
        ...
        add_settings_field(
            ...
            function () {
                ...
                <?php endforeach; ?>
                <p class="description"><?php _e('Selection of command-controllable states', 'hz-discord-bot-tiny'); ?></p>
                <?php
            },
            'hz_discord_bot_tiny_bot',
            'hz_discord_bot_form_section'
        ...
        add_settings_field(
            'hz_discord_bot_form_blocks_message',
            __('Message', 'hz-discord-bot-tiny'),
            function () {
        ...
        add_settings_field(
            ...
            function () {
                ...
                <?php endforeach; ?>
                <p class="description"><?php _e('Status options of the controls that can be selected when an order is queried.', 'hz-discord-bot-tiny'); ?></p>
                <?php
            },
            'hz_discord_bot_tiny_bot',
            'hz_discord_bot_form_section'
        ...
        add_settings_section(
            ...
            __('Phone Application Commands', 'hz-discord-bot-tiny'),
            function () {
                printf(
                    __('You can check your order by <i class="command">%s</i>', 'hz-discord-bot-tiny'),
                    '/phone'
                );
        ...
        add_settings_field(
            ...
            __('Phone Type', 'hz-discord-bot-tiny'),
            function () {
                ...
                $fields = [
                    ...
                    'order_shipping_phone' => __('Shipping Phone', 'hz-discord-bot-tiny'),
                ];
        ...
        add_settings_field(
            ...
            __('Query limit', 'hz-discord-bot-tiny'),
            function () {
                ...
                echo '<p class="description">' . __('Maximum query limit: 10', 'hz-discord-bot-tiny') . '</p>';
            },
            'hz_discord_bot_tiny_bot',
            'hz_discord_bot_form_phone_section'
        );
        ...
        add_settings_field(
            'hz_discord_bot_form_payment_phone',
            __('Payment Gateways', 'hz-discord-bot-tiny'),
            function () {
                ...
                <?php endforeach; ?>
                <p class="description"><?php _e('Selection of command-controllable payment gateways', 'hz-discord-bot-tiny'); ?></p>
                <?php
            },
            'hz_discord_bot_tiny_bot',
            'hz_discord_bot_form_phone_section'
        ...
        add_settings_section(
            ...
            __('Product Enquiry Control Instruction (Beta)', 'hz-discord-bot-tiny'),
            function () {
                printf(
                    __('You can check your product by <i class="command">%s</i>', 'hz-discord-bot-tiny'),
                    '/product'
                );
            },
            'hz_discord_bot_tiny_bot'
        ...
        add_settings_section(
            ...
            function () {
                _e('When the settings page is fully filled out, and after clicking save, if the command does not synchronize, please click manual sync.', 'hz-discord-bot-tiny');
                ...
                foreach ($settings as $key => $value) {
                    if (!empty($value) && $key === 'bot_token') {
                        echo "<input type='hidden' name='hz_discord_bot_setting_{$key}' value='$value'>";
                    }
                }
                $disable = (in_array('', $settings, true)) ? 'disabled' : '';
                echo '<br><br><button type="button" class="button" id="sync_commands" ' . $disable . '>' . __('Sync command', 'hz-discord-bot-tiny') . '</button>';
            },
            'hz_discord_bot_tiny_bot'
        );
    }
    ...
    private function _register_setting_webhook()
    {
        ...
        add_settings_section(
            ...
            function () {
                _e('You can use the Discord Webhooks feature to get notifications quickly and easily.', 'hz-discord-bot-tiny');
                include_once HZ_DISCORD_BOT_TINY_PATH . 'admin/partials/template/base_message_handler.php';
            },
    ...
    private function _register_setting_notify()
    {
        ...
        add_settings_section(
            ...
            function () {
                _e('You can use this function to send messages when an order changes to a specified status and control', 'hz-discord-bot-tiny');
                include_once HZ_DISCORD_BOT_TINY_PATH . 'admin/partials/template/base_message_handler.php';
            },
        ...
        add_settings_field(
            ...
            function () {
                ...
                <?php endforeach; ?>
                <p class="description"><?php _e('Selection of controllable states', 'hz-discord-bot-tiny'); ?></p>
                <?php
            },
            'hz_discord_bot_tiny_notice',
            'hz_discord_bot_notice_form_section'
    ...
    private function _register_setting_setting()
    {
        ...
        add_settings_section(
            ...
            __('Form Section Settings', 'hz-discord-bot-tiny'),
            function () {
                printf(
                    '<p>%s <a href="https://discord.com/developers/applications" target="_blank">Discord Developer Portal</a> %s</p>',
                    __('Please go to', 'hz-discord-bot-tiny'),
                    __('to create an application and obtain the APPLICATION ID.', 'hz-discord-bot-tiny')
                );
                $interactions_url = get_rest_url(null, 'heizhu/v1/interactions');
                echo __('Your Interactions Endpoint URL', 'hz-discord-bot-tiny') . ': ';
                echo '<span class="interactions-url">' . $interactions_url . '</span>';
            },
        ...
        add_settings_field(
            ...
            function () {
                ...
                echo '<input type="text" style="width:50%" name="hz_discord_bot_setting_application_id" value="' . esc_attr($hz_discord_bot_setting_pplication_id) . '">';
        ...
        add_settings_field(
            'hz_discord_bot_setting_public_key',
            __('PUBLIC KEY', 'hz-discord-bot-tiny'),
            function () {
                $hz_discord_bot_setting_public_key = get_option('hz_discord_bot_setting_public_key', '');
                echo '<input type="text" style="width:50%" name="hz_discord_bot_setting_public_key" value="' . esc_attr($hz_discord_bot_setting_public_key) . '">';
            },
            'hz_discord_bot_tiny_setting',
            'hz_discord_bot_setting_form_section'
        );
        add_settings_field(
            'hz_discord_bot_setting_bot_token',
            __('BOT TOKEN', 'hz-discord-bot-tiny'),
            function () {
                $hz_discord_bot_setting_bot_token = get_option('hz_discord_bot_setting_bot_token', '');
                echo '<input type="text" style="width:50%" name="hz_discord_bot_setting_bot_token" value="' . esc_attr($hz_discord_bot_setting_bot_token) . '">';
            },
            'hz_discord_bot_tiny_setting',
            'hz_discord_bot_setting_form_section'
        );
        add_settings_field(
            'hz_discord_bot_setting_guild_id',
            __('GUILD ID', 'hz-discord-bot-tiny'),
            function () {
                $hz_discord_bot_setting_guild_id = get_option('hz_discord_bot_setting_guild_id', '');
                echo '<input type="text" style="width:50%" name="hz_discord_bot_setting_guild_id" value="' . esc_attr($hz_discord_bot_setting_guild_id) . '">';
                echo '<p class="description">' . __('The GUILD ID is the ID of the server where the bot is located.', 'hz-discord-bot-tiny') . '</p>';
            },
            'hz_discord_bot_tiny_setting',
            'hz_discord_bot_setting_form_section'
        );
        add_settings_section(
            'hz_discord_bot_setting_debug',
            __('Debug', 'hz-discord-bot-tiny'),
            function () {
                printf(
                    '<p>You can see the debug log from <sapn class="debug">%s</span></p>',
                    Hz_Discord_Bot_Global::get_log_file_path()
                );
                print_r(Hz_Discord_Bot_Global::get_log_file_url());
            },
            'hz_discord_bot_tiny_setting'
        );
        add_settings_field(
            'hz_discord_bot_setting_debug',
            __('Error Log', 'hz-discord-bot-tiny'),
            function () {
                $hz_discord_bot_setting_debug = get_option('hz_discord_bot_setting_debug', '');
                echo '<label><input type="checkbox" name="hz_discord_bot_setting_debug" value="1" ' . checked($hz_discord_bot_setting_debug, '1', false) . ' />' . __('Enable', 'hz-discord-bot-tiny') . '</label>';
        ...
        add_settings_field(
            'hz_discord_bot_setting_clear',
            __('Clear the command', 'hz-discord-bot-tiny'),
            function () {
